...
namespace watchtower.Census {
    ...
    public class ItemCollection : IItemCollection {
        ...
        private readonly ConcurrentDictionary<int, PsItem?> _Cache = new ConcurrentDictionary<int, PsItem?>();
        ...
        public async Task<PsItem?> GetByID(int ID) {
            PsItem? item = null;
            lock (_Cache) {
        ...
        private async Task<PsItem?> _GetFromCensus(int ID, bool retry) {
        ...
        private PsItem? _ParseItem(JToken result) {
            ...
            PsItem item = new PsItem() {
                ...
                FactionID = result.GetInt32("faction_id", -1)
            };
